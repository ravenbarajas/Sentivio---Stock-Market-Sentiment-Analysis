...
try:
    if not os.path.exists(symbols_meta_file):
        ...
        # Rename column is_etf to etf to match our new schema
        if 'is_etf' in meta_df.columns:
            meta_df = meta_df.rename(columns={'is_etf': 'etf'})
        ...
        for col in ['nasdaq_traded', 'etf', 'test_issue', 'nextshares']:
            if meta_df[col].dtype == object:  # Only convert if string type
                meta_df[col] = meta_df[col].map({'Y': True, 'N': False, 'y': True, 'n': False})
